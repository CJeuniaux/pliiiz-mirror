...
async function generateAIImage(mapping: SemanticMapping, giftIdea: string): Promise<{url: string; confidence: number} | null> {
  try {
    ...
    if (imageUrl) {
      return { url: imageUrl, confidence: mapping.confidence };
    }
...
async function searchUnsplashImage(mapping: SemanticMapping, giftIdea: string): Promise<{url: string; confidence: number; metadata: any} | null> {
  try {
