...
-- Policies pour stores (drop et recreate pour éviter conflits)
...
-- Policies pour taxonomie
drop policy if exists "Taxonomy is viewable by everyone" on gift_taxonomy;
drop policy if exists "Service role can manage taxonomy" on gift_taxonomy;
...
-- Policies pour synonymes
drop policy if exists "Synonyms are viewable by everyone" on synonyms;
drop policy if exists "Service role can manage synonyms" on synonyms;
