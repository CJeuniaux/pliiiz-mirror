...
ADD COLUMN IF NOT EXISTS avatar_status text DEFAULT 'ok',
ADD COLUMN IF NOT EXISTS avatar_regen_requested_at timestamptz,
ADD COLUMN IF NOT EXISTS avatar_regen_reason text;

-- Ajouter contrainte si pas déjà présente
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'profiles_avatar_status_check'
  ) THEN
    ALTER TABLE profiles 
    ADD CONSTRAINT profiles_avatar_status_check 
    CHECK (avatar_status IN ('ok', 'queued', 'processing', 'failed'));
  END IF;
END $$;
