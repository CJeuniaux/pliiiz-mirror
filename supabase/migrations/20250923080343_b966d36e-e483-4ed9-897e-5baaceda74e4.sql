...
-- This ensures they use the permissions of the querying user, not the view creator

-- Drop and recreate views with explicit security_invoker setting
DROP VIEW IF EXISTS public.my_contacts_view CASCADE;
DROP VIEW IF EXISTS public.v_public_profile_source CASCADE; 
DROP VIEW IF EXISTS public.v_unsplash_rebuild_stats CASCADE;

-- Recreate my_contacts_view as a security invoker view
...
-- Recreate v_public_profile_source as a security invoker view
...
-- Recreate v_unsplash_rebuild_stats as a security invoker view  
...
-- Recreate the build_public_payload_v2 function with proper search path
