-- Relax admin check so it can be executed from SQL Editor (postgres/supabase_admin) or service_role
...
BEGIN
  ...
  LOOP
    ...
    ON CONFLICT DO NOTHING; -- ignore tout conflit unique (ex: pair_key ou (owner_id, contact_user_id))
