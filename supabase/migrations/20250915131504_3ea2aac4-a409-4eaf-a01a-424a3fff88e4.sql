-- Correction URGENTE: RPC get_public_profile_secure obsolète
-- PROBLÈME: Ne retourne que preferences.* au lieu de profiles.global_preferences/occasion_prefs
...
RETURNS TABLE(
  ...
  city text, 
  country text,
  -- ✅ AJOUTER les vraies colonnes depuis profiles
  global_preferences jsonb, 
  occasion_prefs jsonb,
  -- Garder anciennes colonnes pour compatibilité
  wishlist jsonb, 
  food_prefs jsonb, 
  style_prefs jsonb, 
  dislikes jsonb,
...
AS $$
  SELECT 
    ...
    p.city,
    p.country,
    -- ✅ VRAIES données depuis profiles (nouveau système)
    ...
    -- Compatibilité avec ancien système (preferences table)
    ...
    p.updated_at
  FROM profiles p
  LEFT JOIN preferences pr ON pr.user_id = p.user_id  -- ✅ LEFT JOIN pour ne pas perdre de profils
