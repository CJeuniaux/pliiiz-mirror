-- 1) Supprimer les triggers qui insèrent dans public.contacts (doublons)
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN
...
-- 2) Helper: clé canonique pour paire d'utilisateurs
...
-- 3) Supprimer la fonction accept_connection existante avant de la recréer
DROP FUNCTION IF EXISTS public.accept_connection(uuid);

-- 4) Créer la RPC idempotente avec ON CONFLICT ON CONSTRAINT
CREATE FUNCTION public.accept_connection(p_conn_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- 3.2 Construire la clé de paire canonique
  v_pair := public._contact_pair_key(v_req, v_tgt);

  -- 3.3 Créer une seule ligne de contact pour la paire (ordre canonique)
