-- Fix SavePreferences pour nouveaux utilisateurs - corrected version
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'preferences' AND policyname = 'prefs_select_self') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'preferences' AND policyname = 'prefs_insert_self') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'preferences' AND policyname = 'prefs_update_self') THEN
...
-- 2) RPC: api.save_preferences_v2 - simplified and robust
...
BEGIN
  ...
  -- Merge occasions into profiles.occasion_prefs if provided
  ...
  -- Bump global_preferences version and create outbox event
  UPDATE public.profiles
  SET 
    global_preferences = 
      CASE 
