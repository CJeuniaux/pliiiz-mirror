-- Fix contacts table with proper constraints and symmetric relationships
-- Add unique constraint and prevent self-linking
ALTER TABLE public.contacts
  ADD CONSTRAINT contacts_owner_contact_unique UNIQUE (owner_id, contact_user_id);

ALTER TABLE public.contacts
  ADD CONSTRAINT contacts_no_self_link CHECK (owner_id <> contact_user_id);
...
-- Improved RLS policies for contacts (replace existing ones)
DROP POLICY IF EXISTS "Users can view their own contacts" ON public.contacts;
DROP POLICY IF EXISTS "Users can create their own contacts" ON public.contacts;

-- New comprehensive policies for contacts
CREATE POLICY "contacts_read_own_relations"
  ON public.contacts FOR SELECT TO authenticated
  USING (owner_id = auth.uid() OR contact_user_id = auth.uid());

CREATE POLICY "contacts_write_self"
