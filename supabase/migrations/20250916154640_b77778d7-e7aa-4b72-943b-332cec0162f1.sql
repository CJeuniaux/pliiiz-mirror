-- 1) SIGNUP V2 - Correction des données et contraintes
-- Nettoyer d'abord les données problématiques
UPDATE profiles 
...
-- Ajouter contrainte pour first_name non vide
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'profiles_first_name_not_empty'
    ) THEN
        ALTER TABLE profiles ADD CONSTRAINT profiles_first_name_not_empty 
