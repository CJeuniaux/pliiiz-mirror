...
export function DebugGrid() {
  ...
  useEffect(() => {
    ...
    const measurePositions = () => {
      ...
      // Lignes VERTES = ROUGE ± --plz-green-gutter
      const root = document.documentElement;
      const styles = getComputedStyle(root);
      const outer = parseFloat(styles.getPropertyValue('--plz-outer-margin')) || 15;
      const gutter = parseFloat(styles.getPropertyValue('--plz-green-gutter')) || 15;
      ...
      const leftGreenAbsolute = leftRedAbsolute + gutter;                // +15px depuis rouge gauche
      const rightGreenAbsolute = rightRedAbsolute - gutter;              // -15px depuis rouge droite
      ...
      // Log utilitaire
      console.log('[DebugGrid] Container:', Math.round(containerWidth), 'px | Rouge(L,R):', Math.round(leftRedAbsolute), Math.round(rightRedAbsolute), '| Vert(L,R):', Math.round(leftGreenAbsolute), Math.round(rightGreenAbsolute));
    };
