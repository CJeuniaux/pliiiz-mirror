...
export function AccessRequestCreateModal({
  ...
  const emailOK = !hasEmail || emailPattern.test(inviteEmail.trim());
  
  const handleInvite = () => {
    const to = hasEmail ? inviteEmail.trim() : "";
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        <div className="p-2 overflow-hidden">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="pliiz-tabs w-full">
              <TabsTrigger value="existing" className="pliiz-tab">
                Utilisateurs
              </TabsTrigger>
              <TabsTrigger value="invite" className="pliiz-tab">
                Inviter
              </TabsTrigger>
            </TabsList>
              ...
              <div className="space-y-4">
                <div className="bg-white/20 backdrop-blur-sm p-4 rounded-2xl border border-white/30">
                  ...
                  <p className="text-sm opacity-90">
                    Votre ami recevra une invitation à créer son compte PLIIIZ et pourra ensuite partager ses préférences avec vous.
                  </p>
                </div>

                 <div>
                   <label htmlFor="invite-email" className="pliiz-label">Email de votre ami(e)</label>
                   <Input 
                     id="invite-email" 
                     type="email" 
                     value={inviteEmail} 
                   ...
                   />
                    {hasEmail && !emailOK && (
                      <p role="alert" className="text-xs text-red-300 mt-1">
                        Adresse email invalide. Vous pouvez aussi envoyer sans destinataire.
                      </p>
                    )}
                 </div>
                 ...
                 <div className="flex gap-2 pt-2">
                   <Button variant="secondary" onClick={onClose} className="flex-1">
                    ...
                    <Button 
                      ...
                      disabled={!emailOK}
                      className="flex-1 pliiz-btn disabled:opacity-50"
                    >
                      Inviter
                    </Button>
                 </div>
